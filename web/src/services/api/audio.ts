...
function assertAudioConfig(config: AiConfig, model: string) {
    ...
    const channel = localChannelForActiveModel(config);
    if (config.channelMode === "local" && !(channel?.baseUrl || config.baseUrl).trim()) throw new Error("请先配置 Base URL");
    if (config.channelMode === "local" && !(channel?.apiKey || config.apiKey).trim()) throw new Error("请先配置 API Key");
}
