...
export default function AdminLayout({ children }: { children: ReactNode }) {
    ...
    return (
        <Layout hasSider={!isMobile} style={{ height: "100vh", overflow: "hidden", background: antToken.colorBgLayout }}>
            {!isMobile ? (
            ...
            <Drawer placement="left" size={adminLayoutStyle.siderWidth} open={mobileMenuOpen} onClose={() => setMobileMenuOpen(false)} styles={{ body: { padding: 0, position: "relative" } }}>
                {menuNode}
            </Drawer>
            <Layout style={{ background: antToken.colorBgLayout }}>
                <Layout.Header
                    style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: adminLayoutStyle.headerHeight, padding: isMobile ? "0 12px" : "0 24px", background: antToken.colorBgContainer, borderBottom: `1px solid ${antToken.colorBorder}` }}
                >
